...
def test_move(timew):
    assert (
        timew.move(5, datetime(2018, 8, 15, 9, 0, 0))
...
def test_shorten(timew):
    assert (
        timew.shorten(2, Duration(timedelta(minutes=10)))
...
def test_stop(timew):
    ...
    assert (
        timew.stop(tags=["my tag1", "my tag2"])
...
def test_tag(timew):
    assert (
        timew.tag(5, ["new tag", "another tag"])
...
def test_untag(timew):
    assert (
        timew.tag(1, ["new tag", "another tag"])
